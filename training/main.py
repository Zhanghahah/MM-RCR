...
def main():
    ...
    dataset = build_dataset(
        ...
        graph_processor=image_processor,
        tokenizer=tokenizer,
    )
    ...
    def evaluation(model, eval_dataloader):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            acc_loss += loss
            # print_rank_0(f"step: {step}, acc_loss: {acc_loss}", args.global_rank)
        model.train()
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        if eval_loss < best_loss:
            best_loss = eval_loss
        print_rank_0(f"Starting saving model for {epoch + 1}", args.global_rank)
