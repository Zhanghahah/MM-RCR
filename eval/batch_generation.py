...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="path to load parrot feat",
    )
    parser.add_argument(
        "--cache_path",
        default=None,
        type=str,
        help="path to load cache llms"
    )
    parser.add_argument(
        "--image_token_length",
        type=int,
...
def main():
    ...
    for ck_name in args.checkpoint_names:
        ...
        model.graph_flatten = model.graph_flatten.to('cuda')
        model = model.bfloat16()
        print_rank_0(model)
        for idx, ann in enumerate(tqdm(test_data)):
            data = json.loads(ann)
            name = data['input']
            ...
            answer = data['output']
            rxn_idx = data["graph_id"]
            graph_path = os.path.join(args.parrot_feat_path, f"info-{rxn_idx}.pkl")
            graph_info = parse_pickle(graph_path)
            graph_feat = torch.tensor(graph_info[3]).unsqueeze(0).cuda().bfloat16()
            feats.append(graph_feat)
            ...
            full_prompt = TEMPLATE(question, with_image=with_feat, first_message=True, num_images=1)
            full_prompt_ids = tokenizer(full_prompt).input_ids # remove bos token
            if with_feat:
                feat_number = len(feats)
                index = full_prompt_ids.index(image_token_dict[DST.DEFAULT_HUMAN_IMAGE_PRETOKEN])
                full_prompt_ids[index] = image_token_dict[DST.image_mapping_dict[str(feat_number)]]
            ...
            generate_output = model.generate(graph_feat, input_ids,
            ...
            get_results[name].append([question, generate_output[1], answer])
