...
def create_dsvl_model_and_transforms(
    ...
    if 'qwen' in args.vision_model_name_or_path.lower():
        ...
        vis_encoder.load_state_dict(
            torch.load(os.path.join(args.vision_model_name_or_path, 'pytorch_model.bin'), map_location='cpu'),
            strict=True)
...
class DeepSpeedViLModel(nn.Module):
    def __init__(self, vis_encoder,
        ...
        self.graph_flatten = self.build_graph_flatten(self.lang_config.hidden_size // 4)
        self._init_weight()
    ...
    def _init_weight(self):
        ...
        self.graph_flatten.requires_grad_(True)
        if self.pos_embedding is not None:
            self.pos_embedding.requires_grad_(True)

    def build_graph_flatten(self, lang_dim):
        return nn.Sequential(
            nn.Linear(256, lang_dim),  # an example implementation
    ...
    def concat(self, img_proj, lang, attention_mask, input_labels, image_num, do_generation=False):
        ...
        for index in range(len(img_proj)):  # each seq has multi iamges, so we need to use it as index
            ...
            assert len(img_pos_list) == image_num[
                index], "the number of images in the lang and image_num does not match"
            ...
            for img_i, img_pos in zip(cur_img, torch.flip(img_pos_list, dims=(
            0,))):  # do it reversely so that we can easily insert the image
                ...
                attention_mask_full = torch.cat(
                    (attention_mask_pre_img, 2 * torch.ones_like(img_i[:, 0]), attention_mask_post_img), dim=0)

                input_labels_full = torch.cat((input_labels_pre_img.long(),
                                               DST.DEFAULT_LABEL_PADDING_NUM * torch.ones_like(img_i[:, 0],
                                                                                               dtype=torch.long),
                                               input_labels_post_img), dim=0)
        ...
        if self.padding_embedding is None:
            with torch.no_grad():
                self.padding_embedding = self.lang_embed(
                    torch.tensor(self.tokenizer.pad_token_id).to(lang.device).unsqueeze(0)).unsqueeze(0).detach()
            ...
            for tensor in tensor_list:
                if max_len > tensor.size(1):
                    if pad_vec:  # output_lang padding
                        ...
                        padded_tensor = torch.cat([tensor] + [self.padding_embedding] * (max_len - tensor.size(1)),
                                                  dim=1)
        ...
        return torch.cat(output_lang, dim=0), torch.cat(output_attention_mask, dim=0), torch.cat(output_input_labels,
                                                                                                 dim=0)

    def forward(self, graph, lang,
                attention_mask=None,
                input_labels=None,
                image_num=1,
        ...
        if self.vis_encoder_update:
            # update vis encoder
            graph_feature = self.vis_encoder(graph)
            if not isinstance(graph_feature, torch.Tensor):
                graph_feature = graph_feature.last_hidden_state
        else:
            # do not update vis encoder
            with torch.no_grad():
                graph = graph.squeeze(1)
                graph_feature = self.graph_flatten(graph)

                if not isinstance(graph_feature, torch.Tensor):
                    graph_feature = graph_feature.last_hidden_state
        graph_proj = self.projection(graph_feature)

        hidden_states, attention_mask, input_labels = self.concat(graph_proj, lang, attention_mask, input_labels,
                                                                  image_num)
        labels = input_labels
        ...
        if self.pos_embedding is not None:
            ...
            position_ids = torch.arange(past_length, hidden_states.size()[1] + past_length, dtype=torch.long,
    ...
    def generate(self, graph, lang,
        ...
        graph_proj = self.projection(graph_feature)
        hidden_states, attention_mask, input_labels = self.concat(graph_proj, lang, attention_mask, input_labels,
                                                                  image_num=[graph.size(0)], do_generation=True)
        ...
        output = self.lang_decoder.generate(input_ids=None,
                                            ...
                                            # this is the number of tokens you want to generate
                                            **generation_kwargs)
