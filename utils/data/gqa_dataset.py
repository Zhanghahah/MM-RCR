...
class GQADataset(Dataset):
    def __init__(
        ...
        self.per_sample_graph = per_sample_graph
        print('check tokenizer', self.tokenizer)
        self.annotation = []
        for ann_path in ann_paths:
            if annotation_key is None:
                self.annotation.extend(open(ann_path, "r").readlines())
            else:
                self.annotation.extend(open(ann_path, "r").readlines()[annotation_key])
        self.sample_graph = sample_graph
        if self.sample_graph:
            print("randomly sample one annotation for each graph")
            self.annotation = self.parse_annotation(self.annotation)

        self.annotation = DST.random_grouping(self.annotation, self.per_sample_graph)
    ...
    def cat_number(self):
        tmp = len(self.annotation) // self.per_sample_graph
        self.arithmetic_progression_multi_image = [tmp * i for i in range(self.per_sample_graph)]
    ...
    def process_graph(self, ann, data_debug_path=None):
        pass
    ...
    def tokenize(self, text):
        ...
        if self.ignore_instruction:
            ...
            labels = [DST.DEFAULT_LABEL_PADDING_NUM] * len(instruction_token["input_ids"]) + labels[len(
                instruction_token["input_ids"]):]
    ...
    def merge_all_images(self, res_list):
        def find_index_and_replace(input_list, attention_mask_list, labels_list, image_number):
            ...
            labels_list[index] = [DST.DEFAULT_LABEL_PADDING_NUM] * len(
                self.image_token_dict[DST.image_mapping_dict[str(image_number)]])
            # flatten nested list
        ...
        image_number = 0
        original_output = {"input_ids": [], "attention_mask": [], "labels": [],
                           "image": []}  # copy.deepcopy(self.system_instruct)
        ...
        for res in res_list:
            ...
            if self.image_token_dict[DST.DEFAULT_HUMAN_IMAGE_PRETOKEN] in res["input_ids"]:
                image_number += 1
                res["input_ids"], res["attention_mask"], res["labels"] = find_index_and_replace(res["input_ids"],
                                                                                                res["attention_mask"],
                                                                                                res["labels"],
                                                                                                image_number)
